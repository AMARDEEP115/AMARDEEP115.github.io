import "./Navbar.css";
import React from "react";
import { Box, Button, Drawer, DrawerBody, DrawerCloseButton, DrawerContent, DrawerFooter, DrawerHeader, DrawerOverlay, Flex, HStack, IconButton, Spacer, Text, useDisclosure } from "@chakra-ui/react";
import {HamburgerIcon} from "@chakra-ui/icons";

function Navbar(){
    const { isOpen, onOpen, onClose } = useDisclosure();
    const btnRef = React.useRef();
    const [home,setHome]=React.useState(false);
    const [about,setAbout]=React.useState(false);
    const [skil,setSkil]=React.useState(false);
    const [proj,setProj]=React.useState(false);
    const [con,setCon]=React.useState(false);
    return <div id="Home">
        <div id="NavFullScreen">
            <Flex width="100%"  bgColor="yellowgreen" position="fixed" h="20" border="1px" alignItems="center" pl="25px" pr="35px" top="0" right="0" left="0" zIndex="1030">
                <Box fontSize='3xl' fontWeight="extrabold">AMAR DEEP</Box>
                <Spacer />
                <HStack spacing='24px'>
                    <nav style={{display:"flex",gap:"24px",alignItems:"center"}}>
                    <Text fontSize='2xl' fontWeight="normal"><a href="#Home" style={{color:home?"blue":"black"}} onClick={()=>{
                        setHome(true);
                        setAbout(false);
                        setSkil(false);
                        setProj(false);
                        setCon(false);
                    }}>HOME</a></Text>
                    <Text fontSize='2xl' fontWeight="normal"><a href="#AboutMe" style={{color:about?"blue":"black"}} onClick={()=>{
                        setHome(false);
                        setAbout(true);
                        setSkil(false);
                        setProj(false);
                        setCon(false);
                    }}>ABOUT ME</a></Text>
                    <Text fontSize='2xl' fontWeight="normal"><a href="#Skills" style={{color:skil?"blue":"black"}} onClick={()=>{
                        setHome(false);
                        setAbout(false);
                        setSkil(true);
                        setProj(false);
                        setCon(false);
                    }}>SKILLS</a></Text>
                    <Text fontSize='2xl' fontWeight="normal"><a href="#Projects" style={{color:proj?"blue":"black"}} onClick={()=>{
                        setHome(false);
                        setAbout(false);
                        setSkil(false);
                        setProj(true);
                        setCon(false);
                    }}>PROJECTS</a></Text>
                    <Text fontSize='2xl' fontWeight="normal">
                        <a style={{display:"flex",gap:"10px",width:"130px",height:"35px",alignItems:"center"}} href="https://drive.google.com/u/0/uc?id=11pN2MFLhuvhCYY2Tu3geGDQ1hh54icPE&export=download" download="resume">
                            <img style={{width:"25px"}} src="data:image/png;base64," alt="resume"/>
                            RESUME
                        </a>
                    </Text>
                    <Text fontSize='2xl' fontWeight="normal"><a href="#Contact" style={{color:con?"blue":"black"}} onClick={()=>{
                        setHome(false);
                        setAbout(false);
                        setSkil(false);
                        setProj(false);
                        setCon(true);
                    }}>CONTACT</a></Text>
                    </nav>
                </HStack>
            </Flex>
        </div>
        <div id="MediunScreen">
            <Flex width="100%" bgColor="yellowgreen" position="fixed" height="20" pl="25px" pr="35px" alignItems="center">
                <Box fontSize='xl' fontWeight="bold">AMAR DEEP</Box>
                <Spacer />
                <HStack spacing='24px' >
                    <Text fontSize='xl' fontWeight="normal"><a href="#Home"  style={{color:home?"blue":"black"}} onClick={()=>{
                        setHome(true);
                        setAbout(false);
                        setSkil(false);
                        setProj(false);
                        setCon(false);
                    }}>HOME</a></Text>
                    <Text fontSize='xl' fontWeight="normal"><a href="#AboutMe"  style={{color:about?"blue":"black"}} onClick={()=>{
                        setHome(false);
                        setAbout(true);
                        setSkil(false);
                        setProj(false);
                        setCon(false);
                    }}>ABOUT ME</a></Text>
                    <Text fontSize='xl' fontWeight="normal"><a href="#Skills"  style={{color:skil?"blue":"black"}} onClick={()=>{
                        setHome(false);
                        setAbout(false);
                        setSkil(true);
                        setProj(false);
                        setCon(false);
                    }}>SKILLS</a></Text>
                    <Text fontSize='xl' fontWeight="normal"><a href="#Projects"  style={{color:proj?"blue":"black"}} onClick={()=>{
                        setHome(false);
                        setAbout(false);
                        setSkil(false);
                        setProj(true);
                        setCon(false);
                    }}>PROJECTS</a></Text>
                    <Text fontSize='xl' fontWeight="normal">
                        <a style={{display:"flex",gap:"10px",width:"130px",height:"35px",alignItems:"center"}} href="https://drive.google.com/u/0/uc?id=11pN2MFLhuvhCYY2Tu3geGDQ1hh54icPE&export=download" download="resume">
                            <img style={{width:"25px"}} src="data:image/png;base64," alt="resume"/>
                            RESUME
                        </a>
                    </Text>
                    <Text fontSize='xl' fontWeight="normal"><a href="#Contact"  style={{color:con?"blue":"black"}} onClick={()=>{
                        setHome(false);
                        setAbout(false);
                        setSkil(false);
                        setProj(false);
                        setCon(true);
                    }}>CONTACT</a></Text>
                </HStack>
            </Flex>
        </div>
        <div id="SmallScreen">
            <Flex bgColor="yellowgreen" position="fixed" height="70px" pl="20px" pr="30px" ml="-11%" mt="-11%" w="100%" alignItems="center">
                <Box fontSize='xl'>AMAR DEEP</Box>
                <Spacer/>
                <Box>
                    <Button ref={btnRef} as={IconButton} icon={<HamburgerIcon boxSize={7} color="red" />} width="30px" onClick={onOpen} border="2px" borderColor="red" backgroundColor="white">
                    </Button>
                    <Drawer
                      isOpen={isOpen}
                      placement='right'
                      onClose={onClose}
                      finalFocusRef={btnRef}
                    >
                        <DrawerOverlay />
                        <DrawerContent>
                            <DrawerCloseButton />
                            <DrawerHeader></DrawerHeader>
                            <DrawerBody>
                                <Text fontSize="lg" fontWeight="normal" className="SSHover"><a href="#Home">HOME</a></Text>
                                <Text fontSize="lg" fontWeight="normal" className="SSHover"><a href="#AboutMe">ABOUT ME</a></Text>
                                <Text fontSize="lg" fontWeight="normal" className="SSHover"><a href="#Skills">SKILLS</a></Text>
                                <Text fontSize="lg" fontWeight="normal" className="SSHover"><a href="#Projects">PROJECTS</a></Text>
                                <Text fontSize='lg' fontWeight="normal" className="SSHover"><a href="#GitStats">GIT STATS</a></Text>
                                <Text fontSize="lg" fontWeight="normal" className="SSHover"><a href="#Contact">CONTACT</a></Text>
                                <a style={{display:"flex",gap:"10px",border:"1px solid black",width:"120px",height:"35px",alignItems:"center",marginTop:"25px"}} href="https://drive.google.com/u/0/uc?id=11pN2MFLhuvhCYY2Tu3geGDQ1hh54icPE&export=download" download="resume">
                                    <img style={{width:"25px"}} src="data:image/png;base64," alt="resume"/>
                                    <p style={{fontWeight:"500"}}>RESUME</p>
                                </a>
                            </DrawerBody>
                            <DrawerFooter></DrawerFooter>
                        </DrawerContent>
                    </Drawer>
                </Box>
            </Flex>
        </div>
        <div style={{marginTop:"185px",width:"80%",margin:"auto",height:"500px"}}>
            <div id="checkTop" style={{margin:"auto",marginTop:"185px"}}>
                <h1 id="AboutName" style={{display:"flex",gap:"20px",margin:"auto",textAlign:"center"}}>Hello I am <span style={{color:"blue"}}>Amar Deep</span></h1>
                <h1 style={{fontSize:"2rem",textAlign:"center"}}>FULL STACK MERN DEVELOPER</h1>
                <h1 style={{color:"green"}}>A passionate Full-Stack developer Capability of managing clean, elegant and efficient code. Skilled in collaboration, decision making and DS & Algo. Always ready to leran something new and deal with challenges and task.</h1>
                <a style={{display:"flex",gap:"10px",border:"1px solid black",width:"120px",height:"35px",alignItems:"center",marginTop:"25px"}} href="https://drive.google.com/u/0/uc?id=11pN2MFLhuvhCYY2Tu3geGDQ1hh54icPE&export=download" download="resume">
                  <img style={{width:"25px"}} src="data:image/png;base64," alt="resume"/>
                  <p style={{fontWeight:"500"}}>RESUME</p>
                </a>
            </div>
        </div>
    </div>
}

export default Navbar;
